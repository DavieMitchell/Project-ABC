...
// Fri, 30 July 2026 — used in the day view so the weekday is visible
// without needing a second line.
export function formatUKWeekdayLong(dateOrKey) {
  const d = typeof dateOrKey === 'string' ? fromDateKey(dateOrKey) : dateOrKey
  return new Intl.DateTimeFormat('en-GB', {
    weekday: 'short',
    day: 'numeric',
...
// Add (or subtract, with a negative n) whole days to a date-key, returning
// a new date-key. Used by the day-view ← → navigation arrows.
...
// Inclusive list of date-keys between two date-keys (for Report date ranges).
export function enumerateDateRange(fromKey, toKey) {
  const keys = []
